...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        # where the first key is state and the 
        # second key is action
        ...
        if not legalActions:
            ...
            for action in self.getLegalActions(state):
                ...
                if qValue > max_qValue:
                    max_qValue = qValue
                    max_action = [action]
                # check for ties, which we'll randomly break
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if this is the first time we've been to this
        # state, initialize our counter
        ...
        updatedQValue = (1-self.alpha)*currentQValue + self.alpha*newQValue
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        newQValue = reward+self.discount*self.computeValueFromQValues(nextState)
        difference = newQValue - currentQValue

        features = self.featExtractor.getFeatures(state, action)
        for feature in features:
            weight = self.weights[feature]
            newWeight = weight+self.alpha*difference*features[feature]
