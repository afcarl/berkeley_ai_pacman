...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getValue(self, gameState, agentIndex, depth):
        #print "getValue for agentIndex " + str(agentIndex) + " at depth " + str(depth)
        ...
        # increment only when we see another pacman
        if nextAgentIndex == 0:
            nextDepth = depth + 1
        else:
            nextDepth = depth
        ...
        # ghost
            ...
            # don't increment depth if we see another ghost
    ...
    def getMaxValueOverSuccessors(self, gameState, agentIndex, depth):
        #print "Getting max value for agent " + str(agentIndex)
        max_value = float("-inf")
        legalMoves = gameState.getLegalActions(agentIndex)
        if not legalMoves:
            return self.getValue(gameState, agentIndex, depth)
    ...
    def getMinValueOverSuccessors(self, gameState, agentIndex, depth):
        #print "Getting min value for agent " + str(agentIndex)
        min_value = float("inf")
        legalMoves = gameState.getLegalActions(agentIndex)
        if not legalMoves:
            return self.getValue(gameState, agentIndex, depth)
