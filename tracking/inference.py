...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # update beliefs for the new evidence
        # i.e. take P(x_t | e_{1:t-1}) and observe to get to P(x_t | e_{1:t})
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        # i.e. take P(x_{t-1} | e_{1:t-1}) and update time to get to P(x_t | e_{1:t-1})
        # we're calling P(x_{t-1} | e_{1:t-1}) oldBelief below
        # this will store a counter we can sum over keyed on new position
        oldBeliefDicts = util.Counter()
        for oldPos, oldBelief in self.beliefs.items():
            # get the distribution over next ghost positions given the motion model
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                # if this is the first term for newPos, make a new counter
                if newPos not in oldBeliefDicts:
                    oldBeliefDicts[newPos] = util.Counter()
                # otherwise, P(x_{t-1} | e_{1:t-1}) * P(x_t | x_{t-1})
                oldBeliefDicts[newPos][oldPos] = oldBelief*prob

        # now we need to do the sum over x_{t-1}
        for newPos, oldBeliefDict in oldBeliefDicts.items():
            self.beliefs[newPos] = oldBeliefDict.totalCount()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for pos in self.legalPositions:
            self.particles = self.particles + [pos]*particlesPerPos
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        particle_counts = util.Counter()
        particle_counts.incrementAll(self.particles, 1)
        particle_counts.normalize()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        nOptions = len(self.legalPositions)**self.numGhosts
        particlesPerPos = self.numParticles/nOptions
        for pos in itertools.product(self.legalPositions, repeat=self.numGhosts):
            # pos will now be a tuple over ghost positions
            self.particles = self.particles + [pos]*particlesPerPos
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:        
            prodEmissionProb = 1
            # for each particle, iterate over all of the ghost
            # positions
            for i in range(self.numGhosts):
                # handle the edge case where we eat the
                # ghost and definitely send it to jail
                ...
                if noisyDistances[i] is None:
                    # make a copy of the full particle
                    newParticle = list(particle)
                    # and put the relevant ghost in jail
                    for tuple_index in range(self.numGhosts):
                        if tuple_index == i:
                            newParticle[tuple_index] = self.getJailPosition(i)
                        else:
                            newParticle[tuple_index] = particle[tuple_index]
                    particle = tuple(newParticle)

                # now weight each particle
                else:
                    trueDistance = util.manhattanDistance(particle[i], pacmanPosition)
                    # p(e_t | x_t)
                    emissionProb = emissionModels[i][trueDistance]
                    prodEmissionProb = prodEmissionProb*emissionProb

            # we can't just assign the key here, because
            # we have the sample particle repeated multiple times,
            # so we have to increment
            new_particles.incrementAll([particle], prodEmissionProb)
    ...
    def elapseTime(self, gameState):
        ...
        newParticles = []
        for particle in self.particles:
            prevGhostPositions = list(particle) # A list of ghost positions
